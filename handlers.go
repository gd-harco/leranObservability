...
func (s *server) handlerShortenLink(w http.ResponseWriter, r *http.Request) {
	...
	s.accessLogger.Println("Shortening URL:", longURL)
	u, err := url.Parse(longURL)
	if err != nil || u.Scheme == "" || u.Host == "" {
		http.Error(w, "invalid URL: must include scheme (http/https) and host", http.StatusBadRequest)
		return
	}
	s.accessLogger.Printf("Parsed URL: scheme=%s, host=%s", u.Scheme, u.Host)
	...
	s.accessLogger.Printf("Generated short code: %s for URL: %s", shortCode, longURL)
...
func (s *server) handlerRedirect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			...
			s.accessLogger.Printf("failed to lookup URL: %v", err)
...
func (s *server) handlerListURLs(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.accessLogger.Printf("failed to list URLs: %v", err)
